...
type routeRecorder struct{ managed, isolated, hooks int }
...
func TestStepRouterSeparatesChangeModesAndHooks(t *testing.T) {
	...
	router := StepRouter{Managed: recorder, Isolated: recorder, Hooks: recorder}
	...
	if recorder.managed != 1 || recorder.isolated != 1 || recorder.hooks != 2 {
