...
ProofRail 端口接受版本化 context envelope：任务契约、父快照、已确认决策、最新失败证据、未决票据、预算、授权 target、允许副作用及引用摘要；返回外部执行事实和候选引用，不返回可信的 task PASS。`agent-runner-request` 以 `requestId/runId/taskId/stepId/attempt` 标识调用，以 `workspaceHash/contextHash/parentSnapshotHash/authorizationHash/budgetHash` 绑定不可变输入，并显式列出 `allowedTargets/allowedEffects`。`authorizationHash` 必须精确绑定且唯一定位一条 authorization grant `recordHash`；准入先按该 hash 找到唯一 grant，再按其 `authorizationId` 在授权账本验证 active/unrevoked/unexpired，并要求 `runId/taskId/stepId/allowedTargets` 与请求绑定一致。`budgetHash` 必须精确绑定该 request 的 cost `reservationHash`，不是 `ledgerHash` 或 allocation hash；该 reservation 必须仍 outstanding（不存在 settlement），且 `runId/requestId/authorizationHash` 与请求一致。`allowedEffects` 当前是操作 effect ID，而 grant `effectClasses` 是风险分类，两者在版本化映射契约冻结前不得直接比较或猜测映射；依赖该映射的 AgentRunner production admission 保持阻断，不得宣称 full admission 已完成。这些绑定语义当前仅由 contracts semantic fixtures 冻结；在 Go production admission 真正落地并通过对应验收前，一律视为阻断项。`mode=create` 不得携带 prior session，其首个事件建立该 request 唯一的 session 绑定；`mode=resume` 必须绑定 `priorSessionId` 与 `priorCompletionHash`。每个 attempt 有固定关联身份；启动前持久化请求和授权摘要，同 requestId 同摘要可重放，不同摘要冲突，重复请求不得重复启动未知状态的代理或应用同一候选。新业务 attempt 产生新身份并计费；恢复必须绑定原 attempt、会话和工作区，无法证明连续性时从持久 envelope 建立新 attempt，不猜测续接成功。缺失、重复、异摘要、已结算、过期/撤销或 scope 不匹配一律 fail-closed。
