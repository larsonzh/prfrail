...
dispatch 的 replay 处置规则：仅 R（`unknown-block`）不得 launch；launch-intent 已存在时不得重发或 relaunch；intent+identity 证明已启动，不得 relaunch；R+C 为终局，永不 launch。唯一允许的同进程重试是启动端口按契约保证未 spawn 的失败返回（例如 Start 返回 error），但不得因此重发 launch-intent。接管、结算与终局发布留给后续切片。

`launches/` 与 `requests/`、`completions/` 适用同一路径安全、所有权与同步纪律：生产构造器创建或确认它，运行期读写入口复检其组件级 symlink/reparse，标记缺失时非空即 corruption。本切片不结算、不发布 completion、不接 Engine、不运行真实候选；T027 仍为 `BLOCKED / NOT IMPLEMENTED`，AT-23 未通过。
