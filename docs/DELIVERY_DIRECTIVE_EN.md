...
Version: v1.14. Date: 2026-09-24. Status: **CN body frozen** ( `_EN` mirror in `docs/DELIVERY_DIRECTIVE_EN.md`).
...
| v1.14 | 2026-09-24 | **Ledger registration (step 0 of slice `DR-1`; authorised by the user 2026-09-24)**: §12.4 registers **OB-46 / OB-47 / OB-48 / OB-49 / OB-50** - respectively the G6-5 region-endpoint dependency, the `_EN` word-list selection rule, the freedom-list flush timing, the `tmp/` lifecycle and the `local-only` exemption, and the `sol-orchestrator` disposition with the generator-inclusion condition. **No new criterion, no change to any rule semantics, no change to the §6.3 criterion scope**; OB-50 gets its own row per the user's ruling (not merged into a same-family row). This row is a **by-product of the `DR-1` slice**, not a standalone directive-governance slice. | `DR-1` (write-back required: this slice's ledger and report must state that v1.14 is a by-product) |
---
...
| OB-46 | 2026-09-24 | **G6-5's region endpoint relies on the `### C.1` heading staying stable**: G6-5 judges only added lines inside "the §0.3 table region plus the Appendix C ledger region (from the top up to the `### C.1` heading)"; once that endpoint heading is renamed or moved the region shifts, and the criterion itself has no assertion that could notice (an empty region passes the sub-check quietly). | **To be discussed**: whether to add an endpoint self-check to G6-5 (a missing endpoint must fail rather than degrade to an empty region); evaluate together with `GATES-TIGHTEN` |
| OB-47 | 2026-09-24 | **The `_EN` word list is selected by the `_EN.md` filename suffix**: G6-5 / G6-6 pick the English word list from the filename string rather than from a language declared by the document; renaming the file or adding a third language would mismatch quietly. | **To be discussed**: whether to select by the language marker declared in the document header; evaluate in the next §6.3 revision |
| OB-48 | 2026-09-24 | **The freedom list has no explicit flush step in §5.1**: measured during directive v1.13 - if the list is written after ⑦ then ⑦ cannot audit it (the first occurrence of a §1.7.3 timing deviation); ⑦ recommends "flush it after ⑥ and before ⑦, and include it in ⑦'s input pack". | **To do**: add that step in the next §5.1 revision (with the "after ⑥ / before ⑦" timing and the input-pack declaration) |
| OB-49 | 2026-09-24 | **`tmp/` has no cleanup mechanism and `local-only` is treated as a permanent exemption**: `local-only` was meant for "temporary artifacts that are not archived", yet it has neither an archive target path nor a cleanup point in time ⇒ in practice it becomes a permanent exemption; `tmp/` carries both "review evidence awaiting archival" and "one-off drafts" with no mechanism to tell them apart. | **To do**: define the cleanup timing and the archive target for `local-only` inside `DIRECTIVE-LEDGER-ARCHIVE` (dual theme: ledger archival + `tmp/` lifecycle) |
| OB-50 | 2026-09-24 | **The `sol-orchestrator` disposition and the generator-inclusion condition were never registered in §12.4**: ① historical conclusion - that orchestrator has no automatic orchestration effect on the current master (its own configuration states that a non-Sol master session means manual mode); the only real effect is **visibility** (the master may choose to call it), and calling it carries three risks: its `tools` include `agent` so it forms a call chain (above the depth limit), its `model` is on the §2.4 blacklist, and without an explicit `model` it falls back silently to the session master model; the conclusion is **do not use it**, already reflected as a §2.4 prohibition. ② future-extension condition - the role files under `.github/agents/` are generated from that orchestrator's `_config.yaml`, so if the generator is ever enabled, **the generation source and scripts must be brought under version control first**, otherwise the role set is not reproducible. | **To be discussed**: the conclusion is settled, but when to write the generator-inclusion condition into §2.5 is not; evaluate in the next §12 revision or in a role-set related slice |
