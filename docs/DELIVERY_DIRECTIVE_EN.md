...
| Notes | ① ⑦ round 3 reported **one Low** (`g5a.js` error routing) which was **left unfixed** because this slice's pre-authorized red lines include "fixing G1/G5 along the way"; it is registered as **OB-20**; ② OB-19's boundary annotation (a user's temporary registration unrelated to this slice) is in the report; ③ the `internal/gates` flake is registered separately as **DR-6** per DR disposition boundary ① |
