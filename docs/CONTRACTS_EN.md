...
Agent-reported completion, exit zero or final text means only that external execution ended. ProofRail must prove process-tree termination, rescan the workspace, validate scope/secrets/effects, freeze a candidate, and independently run declared build/test/verify gates before review/promotion. Timeout, disconnection, failed termination, failed session resume, incomplete logs/costs or unknown external effects are uncertain and remain paused; switching sessions for a blind retry is forbidden. AgentRunner request, capability, event and completion bodies use RFC 8785 canonical JSON with the respective `proofrail:agent-runner-request:1\n`, `proofrail:agent-runner-capability:1\n`, `proofrail:agent-runner-event:1\n` and `proofrail:agent-runner-completion:1\n` domain separators to compute `recordHash`. Sets are lexically sorted before hashing and reject duplicates. requestId, probeId, eventId and completionId are their primary deduplication keys: the same key and digest is an idempotent replay, while the same key with a different digest conflicts. Events additionally reject a session/sequence bound to another record or one request drifting across sessions; completions reject different terminal records for one request. A capability report is compatible only when every fixed matrix entry has runtime evidence and is verified. A completed receipt proves only external execution, process stop and evidence capture, never task PASS.
