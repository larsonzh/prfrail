...
The SessionBridge v0.1.1 extension is an independent dependency with a frozen public v1 contract. The ProofRail product integration must embed a file-IPC client for that contract, writing `cmd_<pid>.json` and reading `res_<pid>.json` directly; users must not be required to install or launch the SessionBridge Python, PowerShell, or sh reference clients. The embedded client explicitly uses mode=silent and legacy=false, the same channel directory/target instance, and requestId-bound receipts. Clear same-ID stale results before atomically writing commands. Verify exact wire data against its RFC, goldens and client implementation; explanatory RFC snippets may omit required fields.

Every ProofRail business attempt must derive a non-empty, stable `conversationId`. Later successful turns and transport retries within that attempt reuse the conversation identity so that SessionBridge RFC §5.1 silent history activates automatically. An empty `conversationId` is allowed only for explicitly stateless diagnostics and must not be used for a continuing AI job. A new attempt uses a new conversation identity; reset sends `resetHistory=true` explicitly. Validate `turnId`, history truncation, and receipts against SessionBridge v1. Conversation history supplies model context rather than authoritative ProofRail state; after host restart, truncation, or loss, reconstruct from the durable context envelope.

When silent execution needs operator intervention, notification, response, and control return belong to ProofRail's own CLI/TUI and do not depend on SessionBridge `@sbr-review`, chat-participant registration, or `reply_<conversationId>.json`. A structured AI intervention request first moves the task/step to `WAITING_FOR_OPERATOR`. ProofRail validates and persists operator input as the applicable operator-interaction, review, authorization, or handoff record; chat text cannot directly change state or authority. Without a valid record, execution remains paused. Resuming silent within the same attempt reuses `conversationId`, uses a new `requestId`, and carries the confirmed conclusion plus record hashes. The general-clarification `operator-interaction` wire contract is not frozen yet; implementation requires its Schema, positive/negative fixtures, hash rules, and replay contract first.
