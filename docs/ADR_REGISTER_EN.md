...
| ADR-002 | Freeze Schema 2020-12 toolchain, canonical JSON/paths/errors/config precedence | Evaluate a mature RFC 8785 implementation; sorted keys alone are not canonical; validator independent of core | Partially frozen: chain/hook/target/workspace/state-event/error/snapshot-manifest/evidence-manifest/review-receipt/promotion-receipt/handoff-receipt/hook-result schemas, JCS, UTF-8 without BOM, version/path, snapshot and pre-review evidence-root hash domains, review policy/waiver authorization elements, the manual-handoff step discriminator and handoffPolicy, the hook execution/gate/disposition split, initial canonical vectors, transitions and category exit codes are set; cross-record checker, independent fixtures/validator, concrete error catalog and remaining objects await T002/T003 |
...
| ADR-004 | SessionBridge v0.1.1 silent plus file queue, core durable idempotency | No fork, visible/auto or GUI fallback; cache is not exactly-once | Partially frozen: shared envelope, one-record-per-file framing, atomic claim, generation fencing, result and dispatch/takeover receipts are set; cross-record checker, signature receipt, capability matrix and platform crash probes await T002/T004 |
