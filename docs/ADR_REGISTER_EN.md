...
| ADR-002 | Freeze Schema 2020-12 toolchain, canonical JSON/paths/errors/config precedence | Evaluate a mature RFC 8785 implementation; sorted keys alone are not canonical; validator independent of core | Partially frozen: chain/hook/target schemas, JCS, UTF-8 without BOM, version/path foundations are set; validator, vectors, exit codes and remaining objects await T002/T003 |
