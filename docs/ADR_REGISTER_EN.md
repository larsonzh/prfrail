...
| ADR-012 | Formal AI execution uses the core-owned `AgentRunner` port and pinned CLI Agent adapters; an explicitly selected `supervised-black-box` candidate intake is also supported, with SessionBridge only bridging messages/the visible UI | Silent generates text only and visible proves panel delivery only. Black-box mode warrants isolated artifact checks and independent acceptance, not process, network, cost or external effects. Reject rebuilding the full VS Code Agent, automatic downgrade, and Agent-private transcripts/completion/exit zero as authority | `APPROVED / PARTIALLY IMPLEMENTED`; the four T026 wire/hash/replay contracts and capability matrix were frozen on 2026-09-10, and the GitHub Copilot CLI 1.0.83 native executable digest is pinned. The first two authorized model probes verified six capabilities. One authorized deny probe on 2026-09-11 proved that a compound PowerShell command bypasses prefix tool filtering, so `toolControl=unsupported`. The matrix is now six verified, one unsupported, and five unknown; the candidate remains blocked and requires replacement/upgrade or a provable external enforcement boundary. T026/AT-23 has not passed, T027 must not start, and T028/AT-24 still closes the black-box flow; S1 remains blocked until then |
