...
| ADR-002 | Freeze Schema 2020-12 toolchain, canonical JSON/paths/errors/config precedence | Evaluate a mature RFC 8785 implementation; sorted keys alone are not canonical; validator independent of core | Partially frozen: chain/hook/target/workspace/state-event/error/error-set/ticket-ledger/snapshot-manifest/evidence-manifest/review-receipt/promotion-receipt/handoff-receipt/hook-result/run-manifest/signature-receipt schemas, JCS, UTF-8 without BOM, version/path, 37 error codes and primary ordering, failure fingerprints and the three-phase budget, snapshot and pre-review evidence-root hash domains, review policy/waiver authorization elements, the manual-handoff step discriminator and handoffPolicy, the hook execution/gate/disposition split, effective-config origins/runtime bindings, detached Ed25519 statement/receipt hash domains, initial canonical vectors, transitions and category exit codes are set; cross-record checker, independent fixtures/validator and remaining objects await T002/T003 |
