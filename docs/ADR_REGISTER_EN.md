...
| ADR-004 | SessionBridge v0.1.1 silent plus file queue, core durable idempotency | No fork, visible/auto or GUI fallback; cache is not exactly-once | Partially frozen: shared envelope, one-record-per-file framing, atomic claim, generation fencing and result rules are set; dispatch/takeover receipts, capability matrix and platform crash probes await T002/T004 |
