...
| ADR-002 | Freeze Schema 2020-12 toolchain, canonical JSON/paths/errors/config precedence | Evaluate a mature RFC 8785 implementation; sorted keys alone are not canonical; validator independent of core | Partially frozen: chain/hook/target/workspace/state-event schemas, JCS, UTF-8 without BOM, version/path, initial canonical vectors and single-record transition foundations are set; cross-event checker, independent fixtures/validator, exit codes and remaining objects await T002/T003 |
