...
| ADR-002 | Freeze Schema 2020-12 toolchain, canonical JSON/paths/errors/config precedence | Evaluate a mature RFC 8785 implementation; sorted keys alone are not canonical; validator independent of core | Partially frozen: chain/hook/target/workspace/state-event/error/snapshot-manifest/evidence-manifest/review-receipt/promotion-receipt schemas, JCS, UTF-8 without BOM, version/path, snapshot and pre-review evidence-root hash domains, review policy/waiver authorization elements, initial canonical vectors, transitions and category exit codes are set; cross-record checker, independent fixtures/validator, concrete error catalog and remaining objects await T002/T003 |
