...
| AT-22 | BLOCKED | T025 尚未实现：缺少 `operator-interaction` Schema/golden、内嵌 SessionBridge IPC、TUI 交互收件箱及恢复测试 |
...
| 门禁 | 状态 | 证据/缺口 |
|---|---|---|
| 核心任务链与 fail-close 场景 | BLOCKED | 既有 AT-01–AT-14、AT-16–AT-21 通过；新增 T025/AT-22 尚未实现 |
...
4. 先完成 T025 并通过 AT-22，证明无需 `@sbr-review` 的 TUI 人工交互闭环；再用最终 ZIP 复跑教程并绑定支持矩阵/发行说明，最后由产品所有者明确批准 S1 exit。在此之前 T018 与 S1 状态保持未完成。