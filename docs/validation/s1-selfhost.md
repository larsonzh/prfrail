...
5. 本轮整改不等于重新完成两代演练，也不代表首次 GitHub CI 或真实 runner 攻击演练已通过。首次 verifier 基线仍待独立评审落库，T017 状态保持 `IMPLEMENTED_AWAITING_CI`。
...
6. GitHub CI Windows/Linux 原生矩阵：等待维护者手工 dispatch 首次运行；未运行前 AT-14 与 T017 不标完成。
...
| 输入 | 当前评审值 |
|---|---|
| `candidate_commit` | 待首次 CI：必须与 verifier commit 不同的完整 candidate SHA |
| `verifier_commit` | 待首次 CI：承载已独立评审 workflow/verifier 的当前 `main` SHA |
...
1. 当前未提交实现不能把自身当作可信 verifier。首次落库需独立代码评审建立 verifier 基线；之后由该较早 verifier commit 审核不同的 candidate commit。这是一次性的信任引导，不以候选运行结果替代人工审查。
2. 当前实现不发布安装包，也不授权 `git commit`、`git push`、tag、签名或 GitHub Release。
3. 固定 seed 只读使用；失败时丢弃 candidate，不能覆盖或回滚 seed。
4. CI 首次双平台通过后，补录 verifier/candidate/run 证据并将 T017 标记 `COMPLETE`；若失败则保持阻断。
