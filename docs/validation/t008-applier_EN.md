...
- Ubuntu (2026-09-08 remote regression): using local helper `.vscode/scripts/run-race-on-ubuntu.ps1`, the current workspace snapshot was synced to `10.0.0.199` and executed under Go 1.27.1 + gcc 13.3.0 with `go test -race -count=20 ./internal/applier ./internal/console ./internal/taskdef` and `go test -race -count=1 ./...`; both passed. First pass succeeded (no proxy fallback needed), and remote temporary workspace was cleaned.
...
T008 does not schedule tasks or steps, produce chain projections or receipts, run gates, or promote candidate output to an accepted snapshot; T009 and later tasks retain those responsibilities. Windows `MoveFileExW(...WRITE_THROUGH)` supplies the available platform replacement durability but is not claimed equivalent to Unix directory fsync under power loss; every uncertain recovery remains fail closed. Re-reviewed 2026-09-08: transient `ACCESS_DENIED`/`SHARING_VIOLATION`/`LOCK_VIOLATION` denials on Windows (e.g. brief antivirus/indexer holds) are now retried with a bounded backoff (up to 10 attempts, increasing interval); persistent contention still fails closed and the Unix path is unchanged.
