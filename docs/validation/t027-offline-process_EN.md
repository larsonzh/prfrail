...
- **Remote CI evidence (green)**: GitHub Actions run `35076045445` (commit `6db5a10`) is fully green — the Ubuntu leg actually executed the `Race` and `Contract fixtures` steps in the default suite (the Windows leg skips both under the `runner.os == 'Linux'` condition), while the three candidate/bootstrap jobs (Candidate build / Candidate probe / Bootstrap and release evidence) stay skipped by design, exactly as in the previous green run (A5 run `34951615184`).
