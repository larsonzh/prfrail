...
5. This remediation does not rerun the two-generation drill or establish success on GitHub CI or real-runner attack exercises. The first verifier baseline still needs independent review and landing; T017 remains `IMPLEMENTED_AWAITING_CI`.
...
6. Native GitHub CI Windows/Linux matrix: pending the first maintainer-dispatched run; AT-14 and T017 remain incomplete until it passes.
...
|---|---|
| `candidate_commit` | Pending first CI: full candidate SHA different from verifier commit |
| `verifier_commit` | Pending first CI: current `main` SHA containing the independently reviewed workflow/verifier |
...
1. The current uncommitted implementation cannot treat itself as a trusted verifier. Its first landing requires independent code review to establish a verifier baseline; that earlier verifier commit then evaluates a different candidate commit. This one-time trust bootstrap does not substitute candidate execution for human review.
...
4. After the first dual-platform CI success, record verifier/candidate/run evidence and mark T017 `COMPLETE`; any failure keeps the gate closed.
