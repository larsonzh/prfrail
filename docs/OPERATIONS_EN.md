...
Core offline mode does not require VS Code. For ProofRail to connect to Copilot Chat through VS Code, separately install VS Code 1.82+, a usable GitHub Copilot Chat extension, and SessionBridge 0.1.1, with matching channel directory and target instance and non-legacy `silent` mode. See the SessionBridge [release page](https://github.com/larsonzh/sessbridge/releases). These are necessary AI-host connectivity prerequisites, but the current S1 candidate has not delivered the executable-step/adapter product loop. An installed extension or reachable message transport is not ProofRail AI-coding acceptance.
