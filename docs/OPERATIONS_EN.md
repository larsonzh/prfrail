...
Date: 2026-09-11; S1 operational design. No formal ProofRail release package exists. The S1 model of manually extracting a Windows amd64 portable ZIP, invoking it by explicit path, and not modifying PATH is approved; see the candidate [installation guide](INSTALLATION_EN.md). The CLI baseline and delivery foundation now provide `version/init/validate/config explain/preview/approvals/interactions/cost report/secret/ai check/run/report` plus internal export modules (`internal/snapshot/export.go`, `internal/evidence/delivery.go`). `preview` is an offline read-only static report and does not execute commands, network probes, model calls, or credential reads. `ai check` is a separately authorized live single-request availability probe, not AgentRunner. `run` currently supports noop-only chains and fail-closes with a non-zero exit for executable `code/build/verify` steps. Remaining product capabilities are still planned RFC interfaces. See the [installation plan](INSTALLATION_PLAN_EN.md) for unresolved release binding and [business workflows](BUSINESS_WORKFLOWS_EN.md) for the complete user flow.
...
T025 provides focused terminal interaction: `interactions list [--ledger <path>] [--json]` rebuilds the pending inbox, `interactions respond` writes a structured response bound to interaction/actor/attempt/context/allowed selection, and `interactions tui` presents and selects items in one ANSI-free terminal. The task/step remains `WAITING_FOR_OPERATOR` until the chain control API validates request/response, current binding, and durable state, and returns a resume command only after event writes complete. T027 must still revalidate the real CLI Agent session/process and authority before continuation; the complete unified TUI is not delivered. SessionBridge `@sbr-review` remains standalone diagnostics only.
...
Current copyable flow is `prfrail init --workspace <path>`, `prfrail validate --chain <chain-file>`, `prfrail config explain --chain <chain-file>`, `prfrail preview --chain <chain-file> [--json]`, `prfrail interactions list|respond|tui`, `prfrail cost report [--ledger <path>] [--json]`, `prfrail ai check ...`, `prfrail run --chain <chain-file> [--run-id] [--run-dir]`, and `prfrail report --run-dir <run-dir>`. New users should first complete the side-effect-free [five-step quick start](INSTALLATION_EN.md#3-five-step-quick-start). `preview` reports `previewHash`, permission boundaries, unknowns, and zero call counters in read-only mode; `interactions` handles existing durable requests and does not invoke a model; `cost report` reads the local cost ledger and reports reserved/settled/unknown holds without default telemetry export; `ai check` reads the bound SecretStore credential and makes one live model request; `run` is currently noop-only. Export support is implemented at library level, but a dedicated CLI `export` command remains in later slices; full executable-step gate/adapter integration is also still in later slices.
...
Sort `profiles` by `profileId` and `channels` by `channel`. `prfrail validate`, `config explain`, and `preview` do not read the SecretStore or invoke a model. `config explain` exposes only profile digests and bindings, never `secretRef`. First run `prfrail secret set --ref windows-credential:ProofRail/deepseek` in a secure local terminal. After authorization for one live request, run `prfrail ai check --chain <chain-file> --channel agent-runner-cli --copilot <pinned-executable> --workspace <isolated-directory> --max-requests 1 --out <record-path> --json`. Exit 0 requires exact `PONG`, zero tool calls, one request, and complete result/usage evidence. Every verdict is first published as canonical JSON without replacing an existing `--out` target. `unavailable/unknown` exits 1 and never switches profile, model, or channel. Later, use `prfrail ai verify --chain <chain-file> --record <record-path> [--prior-record <older-record>]... --channel agent-runner-cli --max-age <duration> --max-requests 1 --json` to verify the configuration digest, channel, freshness, and budget offline. Each `--prior-record` is decoded strictly; replaying the same digest for a `probeId` is idempotent, while a different digest is a blocking fork. Verify neither reads the SecretStore nor invokes a model. These commands prove only the recorded point-in-time CLI-channel availability, not AgentRunner enforcement or AT-23.
