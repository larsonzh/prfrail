...
- [x] T026 [Plan:P1.8] 冻结 `AgentRunner` request/capability/event/completion Schema、canonical/hash、正反 golden、去重/重放规则和 adapter 能力矩阵；对至少一个候选 CLI Agent 固定版本/摘要并实测非交互、cwd、事件/完整日志、session create/resume、取消/进程树停止、工具/网络/权限、用量与确认行为；依赖：T025；验收：AT-23 契约/能力部分；禁止：生产 adapter、付费调用无授权、以帮助文本或提示词代替运行证据。状态：`COMPLETE / CANDIDATE INCOMPATIBLE`。四类 Schema、96 个契约用例、RFC 8785 域分隔摘要、严格绑定/重放冲突与 GitHub Copilot CLI 1.0.83 固定摘要探针均已完成。最终矩阵为 10 verified、2 unsupported、0 unknown：受管 Job Object 验证取消；allow-all 对照证明 `gci` 别名绕过工具 deny、shell `Invoke-WebRequest` 绕过 URL deny。十次追加调用授权使用 7 次，确认 6 premium requests 加取消 unknown，3 次未使用。候选整体 blocked，AT-23 未通过；T027 须等待另行冻结并验证的外部强制边界或兼容候选。验证：[中文](validation/t026-agent-runner-contract.md) / [English](validation/t026-agent-runner-contract_EN.md)。
