...
T027 前置状态更新（2026-09-12）：`ai verify` 现可重复接收 `--prior-record`，逐条严格解码历史 availability 记录并执行已有 replay index；同一 `probeId` 的同摘要重放幂等，异摘要分叉 fail-closed。chain 层另新增平台无关的 `AgentRunnerPort`、运行时 admission hook 与显式 admission-first 离线挂点；非法 step、缺 port、缺 admission、准入失败均不得 dispatch，测试锁定 `admit → dispatch` 顺序及错误传播。该挂点尚未接入 Engine/`StepPort.Execute`，不含具体 capability/enforcement/platform/availability/预算准入实现，也不启动进程。上述能力完全离线，不读取 SecretStore、不调用模型，也不提供缺失的外部 enforcement；T027/AT-23 仍保持 blocked。
