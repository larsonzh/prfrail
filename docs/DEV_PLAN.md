...
- [ ] T026 [Plan:P1.8] 冻结 `AgentRunner` request/capability/event/completion Schema、canonical/hash、正反 golden、去重/重放规则和 adapter 能力矩阵；对至少一个候选 CLI Agent 固定版本/摘要并实测非交互、cwd、事件/完整日志、session create/resume、取消/进程树停止、工具/网络/权限、用量与确认行为；依赖：T025；验收：AT-23 契约/能力部分；禁止：生产 adapter、付费调用无授权、以帮助文本或提示词代替运行证据。状态：`CONTRACT COMPLETE / RUNTIME PROBE PARTIAL`。2026-09-10 已完成四类 Schema、96 个契约用例中的正反 golden、RFC 8785 域分隔摘要、严格解码、绑定及重放冲突测试，并固定 GitHub Copilot CLI 1.0.83 原生可执行文件摘要。首个授权模型探针验证非交互、事件/完整日志、session create、正常退出进程树停止和用量；第二个授权探针由 `auto` 路由到 `gpt-5.6-luna`，真实执行 `powershell(Get-Location)` 并返回精确隔离 workspace，使 cwd 也 verified。当前 6/12 项 verified；session resume、取消、工具/网络/权限控制及无人值守确认 6 项继续 unknown，整体 blocked，AT-23 未通过且 T027 不得开始。验证：[中文](validation/t026-agent-runner-contract.md) / [English](validation/t026-agent-runner-contract_EN.md)。
