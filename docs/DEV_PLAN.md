...
- [ ] T026 [Plan:P1.8] 冻结 `AgentRunner` request/capability/event/completion Schema、canonical/hash、正反 golden、去重/重放规则和 adapter 能力矩阵；对至少一个候选 CLI Agent 固定版本/摘要并实测非交互、cwd、事件/完整日志、session create/resume、取消/进程树停止、工具/网络/权限、用量与确认行为；依赖：T025；验收：AT-23 契约/能力部分；禁止：生产 adapter、付费调用无授权、以帮助文本或提示词代替运行证据。状态：`CONTRACT COMPLETE / RUNTIME PROBE BLOCKED BY AUTHENTICATION`。2026-09-10 已完成四类 Schema、96 个契约用例中的正反 golden、RFC 8785 域分隔摘要、严格解码、绑定及重放冲突测试；GitHub Copilot CLI 1.0.83 原生可执行文件及摘要已固定，无模型的版本探针通过。第一次授权调用在认证阶段拒绝当前 classic PAT；第二次授权预检仅在子进程移除该 token 后确认没有可回落的 OAuth 凭据，并以 1 退出。第二次 `usage.json` 明确记录 0 请求、0 token、0 API 时长和 0 premium request cost，故未发生模型调用或计费。12 项行为能力继续 fail-close 为 unknown；重试需先完成受支持认证并取得新授权。验证：[中文](validation/t026-agent-runner-contract.md) / [English](validation/t026-agent-runner-contract_EN.md)。
