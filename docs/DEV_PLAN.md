...
T027 前置状态更新（2026-09-12）：`ai verify` 现可重复接收 `--prior-record`，逐条严格解码历史 availability 记录并执行已有 replay index；同一 `probeId` 的同摘要重放幂等，异摘要分叉 fail-closed。该能力完全离线，不读取 SecretStore、不调用模型，也不提供缺失的外部 enforcement；T027/AT-23 仍保持 blocked。
