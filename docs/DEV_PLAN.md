...
T027 前置状态更新（2026-09-12）：`ai verify` 现可重复接收 `--prior-record`，逐条严格解码历史 availability 记录并执行已有 replay index；同一 `probeId` 的同摘要重放幂等，异摘要分叉 fail-closed。chain 层已有平台无关的 `AgentRunnerPort`、运行时 admission hook 与统一 `StepRouter.Execute` 的 admission-first 离线分流；AgentRunner 请求必须同时携带显式 `ExecutionTarget` 与 `AgentRunnerFacts`，任一缺失或不匹配都不得回退到普通 isolated port，非法 step、缺 port、缺 admission、准入失败均不得 dispatch。adapter 层新增离线组合准入，严格绑定 request/facts、adapter candidate、`agent-runner-cli` availability、capability/enforcement/platform、新鲜度与并发安全 replay index；由注入时钟提供唯一评估时刻，当前仅准入 `mode=create`。首次 request 可准入；在尚无持久 dispatch/completion 收敛状态时，同摘要 request replay 也 fail-closed，禁止二次启动。该能力尚未接入 Engine，Engine 也不生成 AgentRunner execution target 或 immutable facts；authorization/budget 当前只绑定摘要，尚未验证授权 active/unrevoked 或成本预留 unexhausted；持久 dispatch/completion ledger 尚未实现，因而不能把安全 replay 收敛为既有结果；resume 尚无 prior completion/session/workspace 连续性验证。上述缺口均是生产接线前硬门槛。当前路径不启动进程、不读取 SecretStore、不调用模型，也不提供缺失的外部 enforcement；T027/AT-23 仍保持 blocked。
