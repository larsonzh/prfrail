...
T027 前置状态更新（2026-09-13）：`ai verify` 可重复接收 `--prior-record`，逐条严格解码历史 availability 记录并执行已有 replay index；同一 `probeId` 的同摘要重放幂等，异摘要分叉 fail-closed。chain 层已有平台无关的 `AgentRunnerPort`、运行时 admission hook 与统一 `StepRouter.Execute` 的 admission-first 离线分流；AgentRunner 请求必须同时携带显式 `ExecutionTarget` 与 `AgentRunnerFacts`，任一缺失或不匹配都不得回退到普通 isolated port，非法 step、缺 port、缺 admission、准入失败均不得 dispatch。adapter 层离线组合准入现严格绑定 request/facts、adapter candidate、`agent-runner-cli` availability、capability/enforcement/platform、新鲜度、authorization grant 与 cost reservation：`authorizationHash` 必须唯一命中 active、未撤销、未过期且 run/task/step/targets 匹配的 grant，`budgetHash` 必须命中 run/request/authorization 三元绑定且没有任何 settlement 的 reservation；授权或预算拒绝不会消费 request replay 身份。成本账本公开读写已加并发保护。由注入时钟提供唯一评估时刻，当前仅准入 `mode=create`。首次 request 可准入；在尚无持久 dispatch/completion 收敛状态时，同摘要 request replay 也 fail-closed，禁止二次启动。Engine 单一执行入口现可调用核心拥有的可选 `StepIntentPreparer`；准备阶段已纳入 RUNNING 崩溃语义，非法 target/facts 组合会在执行前失败，immutable facts 以防御性副本传递，默认路径保持不变。adapter 层的 `AgentRunnerStepIntentPreparer` 可将一个严格验证的持久 request 投影为目标 isolated code step 的 execution target/facts，并对 run/attempt/mode/parent 绑定漂移或 workspace 缺失 fail-closed，非目标 step 保持默认路由。`allowedEffects` 与 grant `effectClasses` 的版本化映射仍未冻结，因此不能宣称 full production admission；持久 dispatch/completion ledger 尚未接入 AgentRunner port，现有通用 `StepResult` 也不能替代 terminal completion receipt，因而不能把安全 replay 收敛为既有结果；resume 尚无 prior completion/session/workspace 连续性验证。上述缺口均是生产接线前硬门槛。当前路径不启动进程、不读取 SecretStore、不调用模型，也不提供缺失的外部 enforcement；T027/AT-23 仍保持 blocked。
