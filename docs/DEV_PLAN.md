...
T027 前置状态更新（2026-09-14，A0 replay-root composition）：在上一段基础上，A0 已完成并经 V4 Pro 预审与 Codex 独立终审/收尾复审 PASS。生产 replay store 只由 `NewAgentRunnerReplayStoreForRun(runRoot, runID)` 从 durable run root 推导 `<runRoot>/agent-runner-replay`，不再接受任意 caller root（任意绝对路径构造器改为包内非导出）；run root 必须已存在且含常规文件 `events/state-events.jsonl` 标记；加载 R/C 的 `runId` 不符按 corruption、写入不符按 root conflict fail-closed；写者根/保护根按双向不相交与包含链规则拒绝，覆盖 8.3 短名、大小写与 `\\?\` 别名；symlink/reparse 组件检查在构造期与每次读写加锁后复检，残余 TOCTOU 窗口已写入契约；`ownership.json` 以 no-replace 单调发布、不依赖发布耐久 `proven`，目录移动/复制或所有权不符 fail-closed，构造失败（含 bootstrap 与低层构造）不遗留 ownership 元数据。验证：Windows 原生 `gofmt`/`go build`/`go vet`/`go test -count=1 ./...`、聚焦 replay 测试、Node contract 4/4 与全部平台反例测试（junction 替换、大小写/短名/`\\?\` 别名、运行期换链、并发首开）通过；Unix 侧测试待授权推送后由 GitHub Actions Ubuntu 回归确认。A0 未引入真实 dispatch，replay 仍未接入 `AgentRunnerPort`/Engine；T027 仍为 `BLOCKED / NOT IMPLEMENTED`，AT-23 未通过。验证：[中文](validation/t027-replay-root-composition.md) / [English](validation/t027-replay-root-composition_EN.md)。
