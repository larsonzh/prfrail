...
T027 前置状态更新（2026-09-14，A1 pure admission）：`AgentRunnerCompositeAdmission` 已收敛为纯 preflight 与不可变绑定，删除 `RequestIndex` 内存消费、`ErrAgentRunnerRequestReplayBlocked` 与 `AgentRunnerRequestIndex` 类型；同键幂等/冲突、first-dispatch 与 replay 身份消费保留给耐久 replay store 与 dispatch 路径，边界已写入 CONTRACTS 双语、`AgentRunnerAdmission` 接口文档与实现结构文档。聚焦测试覆盖同一 admission 重复与并发通过、失败后无消耗状态、同 requestId 异 digest 归属 dispatch/store 及输入不变性断言。V4 Pro 前置分析与预审（补做）及 Codex 终审/整改复审均 PASS；R1 契约补句（admission 非锁、dispatch 须在发布 R 后重确认授权与预算）与 R2–R4 已整改；Linux CI 新增 `-race` 门禁并同步工作流冻结契约。T027 仍为 `BLOCKED / NOT IMPLEMENTED`，AT-23 未通过。验证：[中文](validation/t027-pure-admission.md) / [English](validation/t027-pure-admission_EN.md)。
