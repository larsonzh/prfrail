...
- [ ] T001 [Plan:P0.1] 评审 `docs/PRODUCT_REQUIREMENTS*.md`、`docs/SECURITY*.md`、`docs/ADR_REGISTER*.md`；记录所有者/安全评审者、预算、支持/签名和影子范围；依赖：本包；验收：§17.1/6/8 签署，依赖许可决策可查；禁止：代用户批准。

T001 当前状态：`READY_FOR_OWNER_REVIEW`。双语评审材料、需求/任务映射和决策建议已准备，实际选择见 ADR §4；尚无人签署，checkbox 保持未完成。等待期间仅允许无副作用一致性检查，不把“继续”解释成付费、数据访问、签名、发布或 S1 编码授权。
