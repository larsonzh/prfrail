...
| ADR-002 | 冻结 Schema 2020-12 工具链、canonical JSON/路径/错误/配置优先级 | 建议评估 RFC 8785 成熟实现，禁止自创排序即宣称 canonical；JSON Schema 库独立于核心判定器 | 部分冻结：chain/hook/target/workspace/state-event/error/snapshot-manifest Schema、JCS、UTF-8 无 BOM、版本/路径、快照类型/权限/排除/摘要域、首批 canonical 向量、状态转换与分类退出码基础已定；跨记录 checker、独立 fixture/validator、具体错误目录、其余对象待 T002/T003 |
