...
> 2026-09-11 04:12 ADR-012 实施状态更新：当前 **9 verified、3 unknown、blocked**，新增 sessionResume、permissionControl、unattendedConfirmations 已实测；toolControl、networkControl、cancellation 仍缺充分边界证据。旧 P8 权限类型错误导致的 unsupported 已撤回，已批准架构不变，不强制换候选。下表旧能力数量及候选判断保留为历史快照；以 [T026 当前结论](validation/t026-agent-runner-contract.md) 为准。T026 未完成，T027 不得启动。
...
| ADR-012 | 正式 AI 执行采用核心拥有的 `AgentRunner` 端口和固定版本 CLI Agent adapter；另提供显式选择的 `supervised-black-box` 候选导入，SessionBridge 仅作消息/可见界面桥 | silent 只生成文本，visible 只证明面板投递。黑箱模式只保证隔离后的产物检查与独立验收，不保证过程、网络、费用或外部副作用；拒绝复刻完整 VS Code Agent、自动降级或把 transcript/完成声明/退出 0 当权威事实 | `APPROVED / PARTIALLY IMPLEMENTED`；2026-09-10 已冻结 T026 四类 wire/hash/replay 与能力矩阵，并固定 GitHub Copilot CLI 1.0.83 原生可执行文件摘要。前两次授权模型探针验证 6 项能力；2026-09-11 的一次授权 deny 探针证明复合 PowerShell 命令可绕过前缀工具过滤，`toolControl=unsupported`。当前为 6 verified、1 unsupported、5 unknown，整体 blocked；须更换/升级候选或建立可证明的外部强制边界，T026/AT-23 未通过且 T027 不得开始。T028/AT-24 仍关闭黑箱流程；在此之前 S1 阻断 |
