...
| ADR-002 | 冻结 Schema 2020-12 工具链、canonical JSON/路径/错误/配置优先级 | 建议评估 RFC 8785 成熟实现，禁止自创排序即宣称 canonical；JSON Schema 库独立于核心判定器 | 部分冻结：chain/hook/target/workspace/state-event Schema、JCS、UTF-8 无 BOM、版本/路径、首批 canonical 向量与单记录状态转换基础已定；跨事件 checker、独立 fixture/validator、错误码、其余对象待 T002/T003 |
