...
- **若 B3 日后采纳 C2/C3**：必须先修订 `docs/CONTRACTS{,_EN}.md` 的耐久冻结句（Windows 目录项持久性“既不能强制也不能证明”→ 按实测重写）并完成协议先行，**不得因 B3 通过就自动解锁** `unproven` 与 first-dispatch。
