...
> 2026-09-11 ADR-012 实施状态更新：T026 已完成，最终为 **10 verified、2 unsupported、0 unknown、blocked**。Job Object 取消 verified；allow-all 对照证明工具别名和 shell 网络可绕过 CLI deny，故当前候选不兼容。已批准架构不变；T027 仍须等待另行冻结并验证的外部强制边界或兼容候选，AT-23 未通过。详见 [T026 最终结论](validation/t026-agent-runner-contract.md)。
...
| ADR-002 | 冻结 Schema 2020-12 工具链、canonical JSON/路径/错误/配置优先级 | 采用 `canonicalize` 4.0.0 复算 RFC 8785 向量；Ajv 8.20.0 独立于未来 Go 核心 | T002/T025 已冻结 30 份结构 Schema；当前 86 个手写预期 fixture 与两条 canonical 向量全部通过 |
...
| ADR-011 | ProofRail 的 AI/操作员正式交互由本地 CLI/TUI、chain 控制 API 和持久交互记录闭环；SessionBridge `@sbr-review` 只作可选诊断/人工测试 | chat participant 的注册与可见性受 VS Code/扩展生命周期影响，不能承载权威状态、授权或恢复；复用它会形成第二控制面。代价是先冻结 `operator-interaction` 契约并实现 T025 | `ACCEPTED / IMPLEMENTED`；2026-09-10 T025/AT-22 通过，Schema/golden/hash/replay、聚焦终端收件箱与 Engine 控制 API 已验证；完整统一 TUI 和 AgentRunner 接线仍分别由后续切片/T027 负责 |
| ADR-012 | 正式 AI 执行采用核心拥有的 `AgentRunner` 端口和固定版本 CLI Agent adapter；另提供显式选择的 `supervised-black-box` 候选导入，SessionBridge 仅作消息/可见界面桥 | silent 只生成文本，visible 只证明面板投递。黑箱模式只保证隔离后的产物检查与独立验收，不保证过程、网络、费用或外部副作用；拒绝复刻完整 VS Code Agent、自动降级或把 transcript/完成声明/退出 0 当权威事实 | `APPROVED / PARTIALLY IMPLEMENTED`；T026 已完成四类 wire/hash/replay、能力矩阵及 GitHub Copilot CLI 1.0.83 固定摘要探测。最终为 10 verified、2 unsupported、0 unknown：Job Object 取消通过，工具别名及 shell 网络 allow-all 对照证明 CLI deny 可绕过。候选不兼容；T027 须等待可证明的外部强制边界或兼容候选，AT-23 未通过。T028/AT-24 仍关闭黑箱流程；在此之前 S1 阻断 |
| ADR-013 | Windows 发布耐久候选协议与反证（A7）：枚举 C1 现行基线、C2 `MoveFileEx(MOVEFILE_WRITE_THROUGH)`、C3 目录句柄 `FlushFileBuffers`（`GENERIC_READ`+`GENERIC_WRITE` 与 `FILE_FLAG_BACKUP_SEMANTICS`）、C5 两阶段 marker、C6 预建槽位；排除 `ReplaceFile`、temp 句柄 `FILE_FLAG_WRITE_THROUGH`、TxF | 不预设两阶段 marker 成立；A7 只做进程级反证与 API 可观测性，断电注入属 B3；不改变生产发布原语与冻结契约句 | `EXPERIMENTAL / UNRESOLVED`。**级别与前提必须一起读**：在前提 P（进程级崩溃、无断电、单主机 Windows 11 / NTFS 本卷）下——可见性与仲裁层 **`proven`**（P1–P5：进程内 8 路真并发 + 跨进程同轮并发尝试，跨进程强同步未证；构造性论证 + 观测）；断电耐久 **`unresolved`**（U1–U5）；**被证伪的断言集合 D1–D6 一律 `disproven`**（候选 C2/C3 的断电耐久仍 `unresolved` 且为 B3 注入候选，不等同于“候选已被证伪”）；`observed`/`falsified`/`disproven-by-policy` 仅作证据注释，**不作为等级**。Windows 保持 `unproven` 且 first-dispatch 仍拒绝；C2 与 C3 并列为 B3 注入候选；C5 marker 作为耐久机制 `disproven`。完成条件：B3 真实断电注入通过，**且按 `docs/CONTRACTS{,_EN}.md` §7「发布耐久等级与升级条件」完成协议先行修订并经独立评审**，且 AT-23 通过后，才可升 `ACCEPTED`；解除 `unproven` 与 first-dispatch 锁定受该条款的**解锁边界**约束（只在该平台、该前提覆盖的部署环境内）。**2026-09-21（B3b）**：真实断电注入矩阵（主矩阵 17 格×5 = 85 轮全有效、U4 6 格×3 = 18 轮、0 真孤儿；前提 P′ = VirtualBox Win11B3 / NTFS on SATA-AHCI `useHostIOCache=false` / 记录级 ~1 KiB / 2 s cut 窗口 / N=5）判定 **C2 `proven`**、C1/C3/C2C3 `unresolved`（C3/C2C3 的原因为决定格**机制证据缺失**，属装置 fail-closed，**不等同于“不耐久”**）；候选排序据此更新为 **C2 优先，C3/C2C3 未建立主张**；**Windows `unproven` 与 first-dispatch 拒绝仍未改变**——按“协议先行”，升 `ACCEPTED` 的前置是按 CONTRACTS §7「发布耐久等级与升级条件」修订耐久冻结句（独立切片 **B3c**，该口径修订已于 2026-09-21 完成）；升 `ACCEPTED` 仍需该平台自身完整证据 + 独立评审 + AT-23 通过。参见 `docs/validation/t027-b3b-durability.md`（+ `_EN`）与证据包 `docs/validation/evidence/b3b-2026-09-21/` |
| ADR-014 | **切片 `DR-1` 的 ⑦ 终审模型降级（用户直接裁定）**：本片 ⑦ 由 `deepseek-v4-pro` 承担，**不是** §2.2 白名单的 `gpt-5.3-codex` | 理由：Copilot 预算已用 96%，且本片是业务切片验证；未采纳的替代：保留 `gpt-5.3-codex`（预算不可承受）。登记依据 = 用户 2026-09-24 的裁定原文，按 §12.4 的 OB-37 元规则以 **ADR** 形式独立留痕（该元规则允许 OB 或 ADR） | **已裁（用户直接裁定，仅本片有效，不得作为先例）**。四要素：裁决人 = 用户；日期 = 2026-09-24；理由 = Copilot 预算 96% 与业务切片验证；依据 = 用户同轮裁定原文。**性质标注**：含规则语义变更、超出 §2.2 字面标准、系用户直接裁定。**后续义务**：验证报告须如实标注「本片 ⑦ 由非白名单模型承担，独立性降低」并交 ⑧c 复核；本例外**不推广**（其它切片仍按 §7.5 与 §2.2 白名单执行） |
...
| 2 Schema/黄金/validator | 30 份结构 Schema 各有正反例；当前 86 个 fixture 与两条 canonical 向量通过独立 oracle | 已关闭（T003/T025） |
...
## 5. A7 Windows 发布耐久反证结论（ADR-013）

- **结论（2026-09-16，级别与前提必须一起读）**：在前提 **P = 进程级崩溃、无断电、单主机 Windows 11 / NTFS 本卷** 下——可见性与仲裁层 `proven`（P1–P5，构造性论证 + 观测）；**断电耐久 `unresolved`**（U1–U5）；**被证伪的断言集合 D1–D6 一律 `disproven`**（候选 C2/C3 的断电耐久仍 `unresolved` 并移交 B3）；**Windows 发布耐久保持 `unproven`**，`first-dispatch` 仍被拒绝；T027 仍 `BLOCKED / NOT IMPLEMENTED`，AT-23 未通过。**不得把可见性层的 `proven` 读成断电级保证。**
- **关键实测**：`record-linked` 与 `parent-synced` 在 kill 下观察等价（故 kill 不能证明目录项耐久，这是 A7 无法把任何候选升为 proven 的根因）；并发发布恒单赢家且“杀赢家不解锁二次派发”；**目录句柄 `FlushFileBuffers` 在非提权下可调用成功**（只读句柄被拒）——推翻 T004 以来“目录 Sync 不可用”的强表述；`MoveFileEx(MOVEFILE_WRITE_THROUGH)` 无提权可用、已存在目标映射 `fs.ErrExist`、目标被打开时拒绝覆盖。
- **候选排序（供 B3 注入）**：C2 与 C3 并列首选；C6 为契约修订后的结构性候选；C5 marker **不作为耐久机制**（作为“可见性+一致性”手段亦无增益，已证伪）。
- **级别口径与证据限制**：可见性与仲裁层（P1–P5）的 `proven` 带明确前提“进程级崩溃、无断电”，由构造性论证 + 可复现观测共同支撑；耐久类断言一律 `unresolved`。承载 E4/E6/E9/E10 的 `a7native` 实验**不进 CI**，属单主机证据，需在 Windows 主机手动重跑（`PROOFRAIL_A7_ROUNDS=20 go test -tags a7native -run TestA7Native ./internal/adapters/`）。
- **若 B3 日后采纳 C2/C3**：必须先按 `docs/CONTRACTS{,_EN}.md` §7「发布耐久等级与升级条件」完成协议先行修订（该冻结句已由 B3c 改写：Windows 目录项持久性“**不是平台常量**”，按（平台 × 前提）逐项判定）并经独立评审，**不得因 B3 通过就自动解锁** `unproven` 与 first-dispatch。
- **边界与移交**：断电类断言（候选 C2/C3 的落盘保证、C2 文档措辞歧义、本机存储栈 flush 语义、断电下目录项丢失有序、共享卷 fencing）全部交 B3；临时残留累积为 A7 发现项，不在本切片修。详见验证报告：[中文](validation/t027-windows-publication-durability.md) / [English](validation/t027-windows-publication-durability_EN.md)。