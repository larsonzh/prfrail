...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with six Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event and error. Initial canonical vectors, single-record
transitions, error categories and CLI exits are frozen; the cross-event checker, concrete error catalog/primary ordering,
queue, remaining persistent objects and ADR-004 are still pending. T003's
