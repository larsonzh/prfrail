...
T027 prerequisite update (2026-09-12): `ai verify` now accepts repeatable `--prior-record` inputs, strictly decodes each historical availability record, and applies the existing replay index. Replaying the same digest for one `probeId` is idempotent, while a different digest fails closed as a fork. The chain layer also has a platform-neutral `AgentRunnerPort`, runtime admission hook, and explicit admission-first offline integration point. Tests prevent dispatch for an invalid step, missing port or admission, and rejected admission; they also lock the `admit → dispatch` order and error propagation. This integration point is not wired into Engine/`StepPort.Execute`, implements no concrete capability/enforcement/platform/availability/budget admission, and starts no process. These paths are fully offline, read no SecretStore, invoke no model, and do not provide the missing external enforcement; T027/AT-23 remain blocked.
