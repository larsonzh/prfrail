...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with fourteen Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error, adapter-envelope, adapter-receipt,
snapshot-manifest, evidence-manifest, review-receipt, promotion-receipt, handoff-receipt and hook-result. Initial canonical vectors,
...
contract, the manual-handoff step discriminator with its return receipt, and the hook execution/gate/disposition split are frozen; cross-event/cross-record checkers,
