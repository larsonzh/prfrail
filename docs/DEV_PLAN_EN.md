...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with sixteen Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error, adapter-envelope, adapter-receipt,
snapshot-manifest, evidence-manifest, review-receipt, promotion-receipt, handoff-receipt, hook-result, run-manifest and signature-receipt. Initial canonical vectors,
...
the concrete error catalog/primary ordering, remaining persistent objects and ADR-004 capability probes are still pending. T003's
