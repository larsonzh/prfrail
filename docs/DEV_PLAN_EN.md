...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with eight Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error, adapter-envelope and adapter-receipt. Initial
canonical vectors, single-record transitions, error categories/CLI exits and file-queue framing/claim/result/dispatch/
takeover receipt rules are frozen; cross-event/cross-record checkers, the concrete error catalog/primary ordering, signature
