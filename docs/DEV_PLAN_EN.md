...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with fifteen Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error, adapter-envelope, adapter-receipt,
snapshot-manifest, evidence-manifest, review-receipt, promotion-receipt, handoff-receipt, hook-result and run-manifest. Initial canonical vectors,
single-record transitions, error categories/CLI exits, file-queue framing/claim/result/dispatch/takeover receipt rules,
snapshot boundaries, the pre-review evidence root, the approve/reject/waive review verdict, the atomic promotion reader
contract, the manual-handoff step discriminator with its return receipt, the hook execution/gate/disposition split, and effective run configuration origins/bindings are frozen; cross-event/cross-record checkers,
the concrete error catalog/primary ordering, signature
