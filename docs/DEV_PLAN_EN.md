...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with eleven Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error, adapter-envelope, adapter-receipt,
snapshot-manifest, evidence-manifest and review-receipt. Initial canonical vectors, single-record transitions, error
categories/CLI exits, file-queue framing/claim/result/dispatch/takeover receipt rules, snapshot boundaries, the pre-review
evidence root and the approve/reject/waive review verdict are frozen; cross-event/cross-record checkers, the concrete error
catalog/primary ordering, promotion/handoff/signature
