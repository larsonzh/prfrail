...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with nine Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error, adapter-envelope, adapter-receipt and
snapshot-manifest. Initial canonical vectors, single-record transitions, error categories/CLI exits, file-queue
framing/claim/result/dispatch/takeover receipt rules and snapshot content/parent/portable-restoration boundaries are frozen;
