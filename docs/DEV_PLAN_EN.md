...
> Current T026 status, 2026-09-11 04:12: **nine verified, three unknown, blocked**. All six authorized invocations were used, newly verifying session resume, unapproved-write denial, and unattended confirmation failure handling. Wildcard URL denial failed, exact-URL denial passed, and cancellation was observed without whole-tree containment proof. Confirmed cost is five premium requests plus unknown cancellation cost. Remaining toolControl/networkControl/cancellation and effective-configuration boundaries are in the [current T026 conclusion](validation/t026-agent-runner-contract_EN.md). Proceed with offline supervision/policy design, not automatic additional model calls. Old 6/6, unsupported and replacement claims below are historical snapshots; T026 stays unchecked and T027 must not start.
...
- [ ] T026 [Plan:P1.8] Freeze `AgentRunner` request/capability/event/completion Schemas, canonical/hash rules, positive/negative goldens, dedup/replay, and adapter capability matrix. Pin at least one candidate CLI Agent version/digest and probe noninteractive use, cwd, events/complete logs, session create/resume, cancel/process-tree stop, tool/network/permission controls, usage, and confirmation behavior. Depends: T025. Accept: AT-23 contract/capability portion. Exclude: production adapter, unauthorized paid calls, or help text/prompt promises as runtime evidence. Status: `CONTRACT COMPLETE / RUNTIME PROBE BLOCKED`. On 2026-09-10 the four Schemas, positive/negative goldens within 96 contract cases, RFC 8785 domain-separated hashes, strict decoding, binding, and replay-conflict tests were completed, and the GitHub Copilot CLI 1.0.83 native executable digest was pinned. The first two authorized model probes verified noninteractive use, cwd, events/complete logs, session creation, process-tree stop after normal exit, and usage: 6/12 entries. On 2026-09-11 one authorized `tool-deny` probe proved that a compound command can bypass `--deny-tool=powershell(Get-ChildItem*)`, so `toolControl` is unsupported; session resume, cancellation, network/permission controls, and unattended confirmations remain unknown. The candidate remains blocked, AT-23 has not passed, and T027 must not start. The controlled harness `tools/agent-probe/Invoke-AgentProbe.ps1` now fixes PowerShell 5.1 argument-list and exit-code collection defects; remaining discriminating probes still require per-scenario authorization. Validation: [中文](validation/t026-agent-runner-contract.md) / [English](validation/t026-agent-runner-contract_EN.md).
