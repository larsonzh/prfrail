...
- [ ] T026 [Plan:P1.8] Freeze `AgentRunner` request/capability/event/completion Schemas, canonical/hash rules, positive/negative goldens, dedup/replay, and adapter capability matrix. Pin at least one candidate CLI Agent version/digest and probe noninteractive use, cwd, events/complete logs, session create/resume, cancel/process-tree stop, tool/network/permission controls, usage, and confirmation behavior. Depends: T025. Accept: AT-23 contract/capability portion. Exclude: production adapter, unauthorized paid calls, or help text/prompt promises as runtime evidence. Status: `CONTRACT COMPLETE / RUNTIME PROBE PARTIAL`. On 2026-09-10 the four Schemas, positive/negative goldens within 96 contract cases, RFC 8785 domain-separated hashes, strict decoding, binding, and replay-conflict tests were completed, and the GitHub Copilot CLI 1.0.83 native executable digest was pinned. The first authorized model probe verified noninteractive use, events/complete logs, session create, process-tree stop after normal exit, and usage. A second authorized probe routed `auto` to `gpt-5.6-luna`, executed a real `powershell(Get-Location)` call, and returned the exact isolated workspace, verifying cwd. Six of twelve entries are now verified; session resume, cancellation, tool/network/permission controls, and unattended confirmations remain unknown. The candidate remains blocked, AT-23 has not passed, and T027 must not start. Validation: [中文](validation/t026-agent-runner-contract.md) / [English](validation/t026-agent-runner-contract_EN.md).
