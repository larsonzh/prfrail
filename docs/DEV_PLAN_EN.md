...
T002 status: `IN_PROGRESS`. Common version, ID, path, JCS and UTF-8 rules are frozen, with seven Draft 2020-12
structural schemas for chain, hook, target, workspace, state-event, error and adapter-envelope. Initial canonical vectors,
single-record transitions, error categories/CLI exits and file-queue framing/claim/result rules are frozen; the cross-event
checker, concrete error catalog/primary ordering, dispatch/takeover receipts, remaining persistent objects and ADR-004
capability probes are still pending. T003's
