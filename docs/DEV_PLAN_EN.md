...
- [ ] T026 [Plan:P1.8] Freeze `AgentRunner` request/capability/event/completion Schemas, canonical/hash rules, positive/negative goldens, dedup/replay, and adapter capability matrix. Pin at least one candidate CLI Agent version/digest and probe noninteractive use, cwd, events/complete logs, session create/resume, cancel/process-tree stop, tool/network/permission controls, usage, and confirmation behavior. Depends: T025. Accept: AT-23 contract/capability portion. Exclude: production adapter, unauthorized paid calls, or help text/prompt promises as runtime evidence. Status: `CONTRACT COMPLETE / RUNTIME PROBE BLOCKED BY AUTHENTICATION`. On 2026-09-10 the four Schemas, positive/negative goldens within 96 contract cases, RFC 8785 domain-separated hashes, strict decoding, binding, and replay-conflict tests were completed. The GitHub Copilot CLI 1.0.83 native executable and digest are pinned, and the model-free version probe passed. The first authorized call rejected the current classic PAT during authentication. A second authorized preflight removed that token only in the child process, confirmed that no fallback OAuth credential existed, and exited 1. Its `usage.json` records zero requests, tokens, API duration, and premium request cost, proving that no model call or billing occurred. All 12 behavioral capabilities remain fail-closed as unknown. A retry requires supported authentication and fresh authorization. Validation: [中文](validation/t026-agent-runner-contract.md) / [English](validation/t026-agent-runner-contract_EN.md).
