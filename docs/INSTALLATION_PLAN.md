...
| 决策 | 候选 | 冻结条件/责任 | 当前状态 |
|---|---|---|---|
| S1 发行载体 | Windows amd64 压缩包；后续评估 winget | T017 原生验收、固定 commit、SBOM、许可证清单和校验和完成 | 待定 |
...
T016 已冻结 CLI、配置和当前源码运行方式；T024 已冻结库级备份、升级前阻断、停用和数据保留规则；T017 已实现候选 CI artifact、固定 commit、SHA256SUMS、SBOM/许可证门禁，正等待首次 GitHub CI 双平台运行。签名/attestation 是后续增强。T017 完成前本文保持“规划”，[操作手册](OPERATIONS.md) 继续区分当前可执行与未来设计。