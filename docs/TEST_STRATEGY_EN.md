...
| AgentRunner contract | Deterministic fake plus pinned real CLI Agent capability probe | Launch/events/tool loop/stop/resume/usage and independent acceptance | After offline contracts; real paid calls need authorization |
| Visible host integration | SessionBridge v0.1.1, controlled instance | Auxiliary silent and visible black-box delivery/return without inferred completion | After offline contracts; paid calls need authorization |
...
| AT-22 | REQ-006/007/008/009/016/021/023/024/025/027 | Only structured requests enter WAITING_FOR_OPERATOR; TUI shows allowed responses and rejects wrong actor/attempt/context hash plus stale/duplicate replies; timeout/disconnect/persistence failure stays paused and restart rebuilds the inbox; resume uses the same conversationId and a new requestId; manual writes use handoff, secret-direct avoids model/evidence, and the loop works without `@sbr-review` | adapters/chain/console/evidence; T025 |
| AT-23 | REQ-006/007/008/009/016/023/024/025/026 | Fake AgentRunner contracts and a pinned real CLI capability probe; isolated-workspace tool iteration with version/config/session/process/events/logs/usage; timeout, incomplete logs, uncertain stop/resume pause; exit zero still requires rescan and independent gates/review | adapters/guard/chain/evidence; T026/T027 |
| AT-24 | REQ-007/009/021/023/024/025/026/027 | Before visible black-box launch display and persist bound risk acknowledgment; isolated workspace only and delivery is not completion; explicit return triggers full manifest/diff, scope/secret/file/effect scans and independent gates/review; missing acknowledgment, disconnect, unknown process/cost/external effects block, reports say reduced/unknown, and automatic downgrade is forbidden | console/adapters/chain/guard/evidence; T028 |

AT-16–AT-21 are RFC section 19 S1 minimum acceptance; AT-22 covers AI/operator interaction, AT-23 full AgentRunner execution assurance, and AT-24 reduced-assurance supervised black-box candidates. All start with deterministic fixtures and no paid calls; real host/model calls require separate authorization. S0 T002/T003 freeze records/fixtures only; parse success is not runtime evidence. T018 summarizes all 24 groups. US7–US11 cover trial, delivery, revocation, retirement recovery and black-box candidates; local measurements establish value without assumed savings.
