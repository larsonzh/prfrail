# ProofRail（证轨）项目建议书——AI 无人值守工程通用产品抽取方案

> 状态：项目建议书与历史综合设计来源（2026-09-07）。本文将 whois 项目中沉淀的
...
> whois 中的原始 RFC 保留不动；本文件名为兼容既有链接而保留，是本仓库的项目建议书版本。
>
> 本文已经按 R1–R22 完成合并审计，记录立项背景、范围、设计来源和路线。自 DG-02 起，产品需求、
> 业务流程、架构、协议、安全、验收和实施计划分别以 [文档权威矩阵](DOCUMENTATION_PLAN.md#1-权威与阅读顺序)
> 指定的专项文档为规范性来源；本文对应段落保留为迁移来源和设计理由，不再是全域唯一权威。
>
> 2026-09-06 产品完整性增补：第 19 节细化既有需求的产品闭环及生命周期约束，使用 PC-01–PC-06
> 追踪，不新增 R 编号。S1 最小增补列入第 14/17 节评审与验收；本次文档修订不等于 S1 实施授权。
...
ProofRail 的需求、业务规则和用户流程必须在不阅读 whois 文档、不理解 A/B、D/V 等历史术语的前提下
完整成立。whois 仅作为需求来源、影子迁移样例和验收夹具，不是产品正常流程的隐含规范；任何从 whois
继承但尚未写入分域权威文档或 Schema 的行为，均不得由实现自行推断。
...
2026-09-06 范围细化：S0 同时冻结第 19 节的演练、导出、授权、副作用、预算和生命周期契约；
S1 exit 增加该节 PC-01–PC-06 的最小验收（AT-16–AT-21），不得只完成原执行链便宣称产品可用。
S2 扩展审批/通知集成、报表和数据迁移；S3 商业化服务仅在需求验证后独立决策，不影响本地 CLI。
新增 S1 工作量须在 §17.8 重新确认预算；无授权只推进文档/规格，不自动开展实现。
...
13. 第 19 节 PC-01–PC-06 的 S1 最小范围有对应契约、正反样例、责任模块和预算评审：只读演练无
  隐式执行、已接受结果只导出不回写、授权撤销阻断新副作用、外部副作用可识别、费用预留不重用、
  备份/恢复/停用不丢证据。其运行验收属于 S1 exit；S0 只要求规范与独立样例验证。
...
- S1 提供离线、无 AI、非秘密的小型示例和只读执行计划预览，复用 init/validate/config explain。
  显示源目录/运行目录、读取/写入/网络范围、步骤/阻断门禁、评审点、预算、空间估算、排除项与缺失能力。
- 预览不得调用 AI、hook、插件安装、凭据提供方或网络；连版本探测命令也属于执行，须单独明确授权。
  静态检查不能确定的能力标为“未探测”，不能伪称已通过。显式请求的报告写入只允许到用户指定输出。
- minimal/standard/strict 仅改变可选默认，不可关闭哈希、独立接受、单写者等强制不变式。
  报告明确 planned/observed/verified/unknown，提供失败原因和一个可执行的下一步，不只给状态码。
- S1 接受标准：新用户用随附夹具不花模型费用完成预览；恶意配置中的 hook 不被启动；无色/窄终端
  与 --json 均能得到同一决策事实。S2 再扩展图形向导和终端自然语言本地化。

### 19.2 PC-02：可交付的成果，而非只有内部快照

- S1 支持将已接受快照的源码/文档、变更清单、父/目标摘要、验证与评审引用导出为独立交付包。
  明确区分 task PASSED、chain COMPLETED、交付包已导出和外部产品已发布，四者不可互相推断。
- 交付包写到显式指定、与源/run/store 不重叠的新目标目录；目标已存在或发生别名冲突时拒绝覆盖。
  引擎不自动回写源工作树、不 commit/push，不将 candidate 伪装为可交付结果。
- 导出预先核验对象完整性、版本、秘密与路径；临时写入失败不出现“完成”包。重试校验包身份，不覆盖未知内容。
  排除的 secret、缓存、机器环境和不支持文件类型必须列出，导出不保证这些外部状态可重建。
- 在空白隔离目录离线核验交付包；缺失/篡改拒绝。交付记录是独立新证据，不改写原 review 或已接受快照。
  S2 可评估补丁/外部 PR 集成，仍需外部显式授权，不能引入核心 Git 写权限。

### 19.3 PC-03：授权、撤销与审批可达性

- S1 将运行授权明确绑定主体、run/manifest 摘要、工具/路径/网络范围、预算和有效期；内容来自人批准的策略，
  不能把聊天文本、仓库文件或模型回复提升为授权。先有授权，再允许 agent/hook 执行。
- 每次新副作用和接受前重新检查授权；撤销/过期阻断新投递和发布，并进入既有 PAUSED 流程。
  进行中的进程请求受控停止，不能把撤销描述成已经瞬间停止，须保留停止结果与残余风险。
- S1 单用户使用本机可信操作身份和授权记录，不假称不同模型即独立安全主体；写权限无法分离时按能力拒绝。
  waiver 不能绕过完整性、独占写、进程停机或秘密保护等强制安全不变式。
- S1 TUI/CLI 提供待审批队列和原因/到期/下一动作；重启后可恢复。等待不会自动批准。
  S2 通知/webhook 只传脱敏摘要，失败不改变任务状态；远程审批和 RBAC 另做身份认证设计。

### 19.4 PC-04：外部副作用与恢复真实性

- 文件回滚仅恢复受控文件，不能撤销网络发布、邮件、数据库写入、包上传或设备操作。
  S0 hook/adapter 能力声明必须区分只读、本地可丢弃写和外部/不可逆副作用；声明来自可信策略而非模型自报。
- S1 默认拒绝部署/上传/外部变更；必要只读网络访问须明确 host/port/purpose 与可实施的策略。
  无法证明所需限制可执行则拒绝，不能靠命令名或 dry-run 字样判断安全。
- 已有旧系统/人工外部动作只记录观察与关联引用，不纳入“已回滚”保证。副作用结果不确定时暂停对账，
  不盲目重试，不以换 requestId 避开幂等控制。后续补偿动作也不是原子回滚。
- S1 提供恢复计划与诊断摘要：最后已接受快照、未完成 journal、受管进程证据、未知项和可允许动作。
  诊断默认只读脱敏，不发送模型；活动 run 的检查不得偷删锁/截断日志。备份一致性或清理要求停机时必须先证明。
- S2 经独立 ADR 才支持外部写 runner、幂等外部 API 和补偿协议；S1 不承诺通用部署引擎。

### 19.5 PC-05：成本账本与可解释收益

- S1 在 run 级预算内为每次模型调用预留最坏可界定成本，同时限制调用数、token、墙钟与 attempt。
  费用有币种、价格版本/来源和 estimated/observed/unknown 标识；计费条款不明不能默认为免费。
- 发起调用前持久化预留；成功、失败、超时及重试均可产生费用。超时/崩溃后的未知结算保留占用，
  未对账不得再次使用同一额度；预算增加要新授权并留证，不能重新开 run 偷绕过所有者配置的共享上限。
- 支持无需模型的离线模式；已订阅产品无法获精确单次价格时明确计量限制，采用用户批准的调用/token
  硬上限而不声称保证货币账单。不可控的供应商计费超额不得被宣传为已技术阻止。
- 本地报告至少有已接受任务数、拒评/修复次数、人工介入、等待/执行时长、估算/实际费用及未知余额。
  无默认遥测，不上传仓库或提示。S2 再加跨 run 成本趋势；S3 收费/账户/云管理须单独商业验证。
...
- S1 停用/卸载先停进程、撤销适配器授权并展示保留/删除清单，默认保留 run/store 和审计证据。
  数据删除是独立显式确认动作，审计锁定或有引用的对象不可绕过 GC；不删除共享 SessionBridge 或用户工具链。
- secret 误入不可变证据时，隔离并限制访问、撤销凭据、阻止外发；不得静默改写并重新签名历史。
  依法或依用户要求删除与审计保留冲突时需人工策略决策及不含秘密的处置记录，不承诺绝对永久保留。
- S2 提供显式版本迁移工具、支持期/废弃提醒和经用户确认的更新流程；无人值守静默自更新不是 S1 必需。

### 19.7 成本与范围纪律

上述最小能力扩展现有 console/chain/taskdef/snapshot/evidence/gates/tickets/adapters，不增加商业后台、
账号计费系统、云存储或独立聊天产品。优先使用确定性夹具验证业务结果，不为产品分析调用额外模型。
阶段是否可用要看“安全完成用户目标”，而非功能条目数量；产品收益必须由已接受任务成本和人工介入实测证明。
